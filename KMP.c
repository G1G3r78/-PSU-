...
#define MAX 1000
//таблица(массив) pi - макс. длин совп. преффиксов и суффиксов
void pi_prefix_suffix(char pattern[], int pi[], int strSize){//подстрока, pi - макс. длин совп. префффиксов и суффиксов, длина подстроки
...
int kmp(char text[], char pattern[]) {//поиск индекса начала подстроки
    ...
    for (int i = 0; i < n; i++){
        ...
        if (j == m) { //кол-о совпавших символов совпало
